...
- Schema validation was decoupled from `@lonli-lokli/dynamic-forms-core` to resolve issue [#2693](https://github.com/rjsf-team/react-jsonschema-form/issues/2693). Additionally, in order to break a circular dependency in the validation refactor, the `@lonli-lokli/dynamic-forms-core/utils.js` file was split out into its own `@lonli-lokli/dynamic-forms-utils` package as was suggested in [#1655](https://github.com/rjsf-team/react-jsonschema-form/issues/1655).
- The theme for Material UI version 5 (i.e. `@lonli-lokli/dynamic-forms-mui`) was split out of the theme for version 4 (i.e. `@lonli-lokli/dynamic-forms-material-ui`) to resolve the following issues: [#2762](https://github.com/rjsf-team/react-jsonschema-form/issues/2762), [#2858](https://github.com/rjsf-team/react-jsonschema-form/issues/2858), [#2905](https://github.com/rjsf-team/react-jsonschema-form/issues/2905), [#2945](https://github.com/rjsf-team/react-jsonschema-form/issues/2945)
...
- `@lonli-lokli/dynamic-forms-utils`: All of the [utility functions](../api-reference/utility-functions.md) previously imported from `@lonli-lokli/dynamic-forms-core/utils` as well as the Typescript types for RJSF version 5.
...
- `@lonli-lokli/dynamic-forms-validator-ajv6`: The [ajv](https://github.com/ajv-validator/ajv)-v6-based validator refactored out of `@lonli-lokli/dynamic-forms-core@4.x`, that implements the `ValidatorType` interface defined in `@lonli-lokli/dynamic-forms-utils`.
- `@lonli-lokli/dynamic-forms-validator-ajv8`: The [ajv](https://github.com/ajv-validator/ajv)-v8-based validator that is an upgrade of the `@lonli-lokli/dynamic-forms-validator-ajv6`, that implements the `ValidatorType` interface defined in `@lonli-lokli/dynamic-forms-utils`. See the ajv 6 to 8 [migration guide](https://ajv.js.org/v6-to-v8-migration.html) for more information.
- `@lonli-lokli/dynamic-forms-mui`: Previously `@lonli-lokli/dynamic-forms-material-ui/v5`, now provided as its own theme.
...
If you need to build a custom widget for this kind of `enumOptions`, there are a set of `enumOptionsXXX` functions in `@lonli-lokli/dynamic-forms-utils` to support your implementation.

### `@lonli-lokli/dynamic-forms-core` BREAKING CHANGES

#### Types

In version 4, RJSF exported all its types directly from `@lonli-lokli/dynamic-forms-core`.
In version 5, only the types for the `Form` component and the `withTheme()` HOC are exported directly from `@lonli-lokli/dynamic-forms-core`.
All the rest of the types for RJSF are now exported from the new `@lonli-lokli/dynamic-forms-utils` package.

NOTE: The types in `@lonli-lokli/dynamic-forms-utils` have been improved significantly from those in version 4 and as a result may require you to fix your Typescript typings and add some casts.
...
The new `@lonli-lokli/dynamic-forms-validator-ajv6` package contains the refactored implementation of the version 4 validator; It was provided for backwards compatibility with RJSF v4, and it is deprecated.  
The new `@lonli-lokli/dynamic-forms-validator-ajv8` package contains the refactored implementation of the version 4 validator, that has been converted to use the `Ajv 8` validator and has more capabilities than the `Ajv 6` one. See the [Ajv migration guide](https://ajv.js.org/v6-to-v8-migration.html) for more information.
...
import { RJSFSchema } from "@lonli-lokli/dynamic-forms-utils";
import Form from "@lonli-lokli/dynamic-forms-core";
import { customizeValidator, CustomValidatorOptionsType } from "@lonli-lokli/dynamic-forms-validator-ajv8";
...
```tsx
import { DescriptionFieldProps, RJSFSchema, TitleFieldProps } from "@lonli-lokli/dynamic-forms-utils";
import Form from "@lonli-lokli/dynamic-forms-core";
import validator from "@lonli-lokli/dynamic-forms-validator-ajv8";
...
```tsx
import { ArrayFieldTemplateProps, ErrorListProps, FieldTemplateProps, ObjectFieldTemplateProps, RJSFSchema } from "@lonli-lokli/dynamic-forms-utils";
import Form from "@lonli-lokli/dynamic-forms-core";
import validator from "@lonli-lokli/dynamic-forms-validator-ajv8";
...
```tsx
import { RJSFSchema, SubmitButtonProps } from "@lonli-lokli/dynamic-forms-utils";
import Form from "@lonli-lokli/dynamic-forms-core";
import validator from "@lonli-lokli/dynamic-forms-validator-ajv8";
...
In version 5, all the utility functions that were previously accessed via `import { utils } from '@lonli-lokli/dynamic-forms-core';` are now available via `import utils from '@lonli-lokli/dynamic-forms-utils';`.
Because of the decoupling of validation from `@lonli-lokli/dynamic-forms-core` there is a breaking change for all the [validator-based utility functions](../api-reference/utility-functions.md#validator-based-utility-functions), since they now require an additional `ValidatorType` parameter.
More over, one previously exported function `resolveSchema()` is no longer exposed in the `@lonli-lokli/dynamic-forms-utils`, so use `retrieveSchema()` instead.
...
```tsx
import { RJSFSchema, FieldProps } from '@lonli-lokli/dynamic-forms-utils';
...
```tsx
// Change breaking-change function to schemaUtils instead, otherwise import from @lonli-lokli/dynamic-forms-utils
// import { utils } from '@lonli-lokli/dynamic-forms-core'; <- version 4
// const { isMultiSelect, resolveSchema, getUiOptions } = utils; <- version 4
import { RJSFSchema, WidgetProps, getUiOptions } from '@lonli-lokli/dynamic-forms-utils';
...
Because of the decoupling of validation from `@lonli-lokli/dynamic-forms-core` this file was refactored into its own `@lonli-lokli/dynamic-forms-validator-ajv8` package.
...
### `@lonli-lokli/dynamic-forms-material-ui` BREAKING CHANGES
...
```
{
  ...
  "compilerOptions": {
    ...
    "baseUrl": ".",
    "paths": {
      "@lonli-lokli/dynamic-forms-material-ui/*": ["node_modules/@lonli-lokli/dynamic-forms-material-ui/dist/*"]
    }
  }
}
```
...
```
  "moduleNameMapper": {
    "@lonli-lokli/dynamic-forms-material-ui/v4": "<rootDir>/node_modules/@lonli-lokli/dynamic-forms-material-ui/dist/v4.js"
  },
```

#### Migrating for Material UI version 5

If you were using this theme for Material UI version 5, you will want to use `@lonli-lokli/dynamic-forms-mui` instead.
...
```
  "moduleNameMapper": {
    "@lonli-lokli/dynamic-forms-material-ui/v5": "<rootDir>/node_modules/@lonli-lokli/dynamic-forms-material-ui/dist/v5.js"
  },
```
...
```tsx
import Form5 from '@lonli-lokli/dynamic-forms-material-ui';
```

or

```tsx
import Form from '@lonli-lokli/dynamic-forms-material-ui/v5';
```
...
import { Theme } from '@lonli-lokli/dynamic-forms-material-ui/v5';
...
```tsx
import { withTheme } from '@lonli-lokli/dynamic-forms-core';
import { Theme as Theme5 } from '@lonli-lokli/dynamic-forms-material-ui';
// Make modifications to the theme with your own fields and widgets
