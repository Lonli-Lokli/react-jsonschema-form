...
React Json Schema Form provides a default `@lonli-lokli/dynamic-forms-validator-ajv6` implementation that uses version 6 of the [ajv](https://github.com/ajv-validator/ajv) validator.

> NOTE: The ajv6 validator has been deprecated from the outset in favor of the `@lonli-lokli/dynamic-forms-validator-ajv8` and is provided primarily to assist in code migrations

It also provides a new `@lonli-lokli/dynamic-forms-validator-ajv8` implementation that uses version 8 of the [ajv](https://github.com/ajv-validator/ajv) validator.
...
It is also possible for you to provide your own implementation if you desire, as long as it fulfills the `ValidatorType` interface specified in `@lonli-lokli/dynamic-forms-utils`.
...
In 5.7.0, support for precompiled validators was added to the `@lonli-lokli/dynamic-forms-validator-ajv8` package.
...
The `@lonli-lokli/dynamic-forms-validator-ajv8` package exports the `compileSchemaValidators()` function that does this.
It is expected that this function will be used in a manner similar to the following:

```cjs
const compileSchemaValidators = require('@lonli-lokli/dynamic-forms-validator-ajv8/dist/compileSchemaValidators').default;
...
```cjs
const { compileSchemaValidators } = require('@lonli-lokli/dynamic-forms-validator-ajv8');
...
The `@lonli-lokli/dynamic-forms-validator-ajv8` package exports the `createPrecompiledValidator()` function for this.
Here is an example of how to use your precompiled validator with your `Form`:

```tsx
import { createPrecompiledValidator, ValidatorFunctions } from '@lonli-lokli/dynamic-forms-validator-ajv8';
import Form from '@lonli-lokli/dynamic-forms-core'; // Or whatever theme you use
...
```ts
import { compileSchemaValidatorsCode } from '@lonli-lokli/dynamic-forms-validator-ajv8/dist/compileSchemaValidators';
...
```tsx
import type { ValidatorFunctions } from '@lonli-lokli/dynamic-forms-validator-ajv8';
...
> Notes:
>
> - The `customValidate()` function must implement the `CustomValidator` interface found in `@lonli-lokli/dynamic-forms-utils`.
...
> Notes:
>
> - The `transformErrors()` function must implement the `ErrorTransformer` interface found in `@lonli-lokli/dynamic-forms-utils`.
...
Each element in the `errors` list passed to `transformErrors` is a `RJSFValidationError` interface (in `@lonli-lokli/dynamic-forms-utils`) and has the following properties:
...
```tsx
import { Form } from '@lonli-lokli/dynamic-forms-core';
import { RJSFSchema, ErrorListProps } from "@lonli-lokli/dynamic-forms-utils";
import validator from "@lonli-lokli/dynamic-forms-validator-ajv8";
...
The following props are passed to `ErrorList` as defined by the `ErrorListProps` interface in `@lonli-lokli/dynamic-forms-utils`:
...
To support additional meta schemas, you can create and pass to the `Form` component a customized `@lonli-lokli/dynamic-forms-validator-ajv8`:
...
import { Form } from '@lonli-lokli/dynamic-forms-core';
import { RJSFSchema } from '@lonli-lokli/dynamic-forms-utils';
import { customizeValidator } from '@lonli-lokli/dynamic-forms-validator-ajv6';
...
NOTE: This syntax works only for the `@lonli-lokli/dynamic-forms-validator-ajv6` validator; if you only use the `draft-04` schema, and you want to use the `@lonli-lokli/dynamic-forms-validator-ajv8` you can do the following:
...
To add formats of your own, you can create and pass to the `Form` component a customized `@lonli-lokli/dynamic-forms-validator-ajv8`:
...
```tsx
import { Form } from '@lonli-lokli/dynamic-forms-core';
import { RJSFSchema, ErrorSchema } from '@lonli-lokli/dynamic-forms-utils';
import validator from '@lonli-lokli/dynamic-forms-validator-ajv8';
...
In version 5, with the advent of the decoupling of the validation implementation from the `Form`, it is now possible to provide additional options to the `ajv` instance used within `@lonli-lokli/dynamic-forms-validator-ajv8`.
...
So if your schema is using an older format, you have to either upgrade it or stick with the `@lonli-lokli/dynamic-forms-validator-ajv6`.

Our implementation of `@lonli-lokli/dynamic-forms-validator-ajv8` also utilizes Ajv's internal cache to avoid unnecessarily re-compiling schemas, which can be an expensive operation. The cache key is the schema `$id`.
...
By default, ALL formats are being added to the default `@lonli-lokli/dynamic-forms-validator-ajv8` that you get when you import it.
...
By default, the `@lonli-lokli/dynamic-forms-validator-ajv8` uses the `draft-07` schema version.
