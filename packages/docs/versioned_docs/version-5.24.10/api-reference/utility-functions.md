...
In version 5, the utility functions from `@lonli-lokli/dynamic-forms-core/utils` were refactored into their own library called `@lonli-lokli/dynamic-forms-utils`.
...
The `@lonli-lokli/dynamic-forms-utils` package exports a set of constants that represent all the keys into various elements of a RJSFSchema or UiSchema that are used by the various utility functions.
...
Those types are exported for use by `@lonli-lokli/dynamic-forms-core` and all the themes, as well as any customizations you may build.
...
There are enumerations in `@lonli-lokli/dynamic-forms-utils` that are exported for use by `@lonli-lokli/dynamic-forms-core` and all the themes, as well as any customizations you may build.
...
> NOTE: This is function is deprecated. Use the `validationDataMerge()` function exported from `@lonli-lokli/dynamic-forms-utils` instead. This function will be
...
```ts
import { ErrorSchemaBuilder, ErrorSchema } from "@lonli-lokli/dynamic-forms-utils";
