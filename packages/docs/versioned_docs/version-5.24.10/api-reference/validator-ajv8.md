# @lonli-lokli/dynamic-forms-validator-ajv8 APIs

In RJSF version 5, the original, embedded AJV 6 validator implementation from earlier versions was extracted into its own package, `@lonli-lokli/dynamic-forms-validator-ajv6`, which was immediately deprecated since AJV 6 is no longer getting maintenance updates.
A new `@lonli-lokli/dynamic-forms-validator-ajv8` package was added that uses the AJV 8 package, including adding support for using precompiled validators.
...
There are a few Typescript types that are exported by `@lonli-lokli/dynamic-forms-validator-ajv8` in support of the APIs.
