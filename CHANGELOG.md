...
## @lonli-lokli/dynamic-forms-util
...
## @lonli-lokli/dynamic-forms-chakra-ui
...
## @lonli-lokli/dynamic-forms-core
...
## @lonli-lokli/dynamic-forms-antd
...
## @lonli-lokli/dynamic-forms-semantic-ui

- Updated `DescriptionField` to render the `description` using the `RichDescription` field

## @lonli-lokli/dynamic-forms-shadcn
...
## @lonli-lokli/dynamic-forms-fluent-ui
...
## @lonli-lokli/dynamic-forms-material-ui
...
## @lonli-lokli/dynamic-forms-validator-ajv6

- BREAKING CHANGE: This deprecated validator has been removed

## @lonli-lokli/dynamic-forms-validator-ajv8
...
## @lonli-lokli/dynamic-forms-bootstrap-4

- BREAKING CHANGE: Package has been replaced with `@lonli-lokli/dynamic-forms-react-bootstrap`. `react-boostrap` v1 / Bootstrap 4 are no longer supported in RJSF v6.

## @lonli-lokli/dynamic-forms-material-ui

- BREAKING CHANGE: Removed `@lonli-lokli/dynamic-forms-material-ui` package. Material UI v4 (`@material-ui/core`) has been deprecated since September 2021. To use Material UI v5 (`@mui/core`) with RJSF, please use the `@lonli-lokli/dynamic-forms-mui` theme instead.

## @lonli-lokli/dynamic-forms-react-bootstrap

- Added new package to replace `@lonli-lokli/dynamic-forms-bootstrap-4`
...
- Updated the peer dependencies for `@lonli-lokli/dynamic-forms-*` to be `5.24.x`
...
- Switched uses of `lodash.isEqual()` to `@lonli-lokli/dynamic-forms-utils.deepEquals`.
...
- Switched uses of `lodash.isEqual()` to `@lonli-lokli/dynamic-forms-utils.deepEquals` at precompiledValidator.
...
- Updated the peer dependencies for `@lonli-lokli/dynamic-forms-*` to be `5.23.x`
...
- Bumped peer dependencies to 5.22.x due to updated type definition and API changes in @lonli-lokli/dynamic-forms-utils
...
- Use `@lonli-lokli/dynamic-forms-utils` `deepEquals()` instead of `lodash.isEqual()` to improve performance, fixing [#4291](https://github.com/rjsf-team/react-jsonschema-form/issues/4291)
...
- Updated the peer dependencies to `5.20.x` due to types and API changes in `@lonli-lokli/dynamic-forms-utils`
...
## @lonli-lokli/dynamic-forms-bootstrap4
...
- [#4080](https://github.com/rjsf-team/react-jsonschema-form/issues/4080) - BREAKING CHANGE: Removed the `base64` object from the `@lonli-lokli/dynamic-forms-utils` package. Note that this is a breaking change if you relied on the `base64` object exported by `@lonli-lokli/dynamic-forms-utils`. Since this change caused [#4080](https://github.com/rjsf-team/react-jsonschema-form/issues/4080), and was only internally used by playground code, we are shipping this change in a patch release.
...
- [#4024](https://github.com/rjsf-team/react-jsonschema-form/issues/4024) Updated the base64 references from (`atob` and `btoa`) to invoke the functions from the new `base64` object in `@lonli-lokli/dynamic-forms-utils`.
...
- Removed `dateElementProps` function implementation, and replaced it with `getDateElementProps` from `@lonli-lokli/dynamic-forms-utils`.
...
- However, if users of @lonli-lokli/dynamic-forms-antd want to use v5 styling, they need to wrap your application with the `StyleProvider` from `@ant-design/cssinjs`. They need not have to install this package, its a transitive package coming from antd.
...
- Refactored the `createErrorHandler()`, `toErrorList()`, `toErrorSchema()` and `unwrapErrorHandler()` functions from the `@lonli-lokli/dynamic-forms-validator-ajv6` and `@lonli-lokli/dynamic-forms-validator-ajv8` implementations since they were identical
...
- Removed the refactored functions and constant from the `AJV6Validator` in favor of using the new functions and constant from `@lonli-lokli/dynamic-forms-utils`

## @lonli-lokli/dynamic-forms-validator-ajv8

- Removed the refactored functions and constant from the `AJV8Validator` in favor of using the new functions and constant from `@lonli-lokli/dynamic-forms-utils`
...
- Switched to using `TextField` for the `WrapIfAdditionalTemplate` label key input to match the `@lonli-lokli/dynamic-forms-mui` fix
...
- Updated the `AltDateTimeWidget` in `@lonli-lokli/dynamic-forms-core` to add `className="list-inline-item"` to the `LI` tags
...
- Updated the `peerDependencies` in all packages to remove the `-beta.x` tags from the `@lonli-lokli/dynamic-forms-xxxx` packages
...
## @lonli-lokli/dynamic-forms-playground

- Fix Vite development server [#3228](https://github.com/rjsf-team/react-jsonschema-form/issues/3228)

## @lonli-lokli/dynamic-forms-validator-ajv8

- BREAKING CHANGE: Disable form data validation for invalid JSON Schemas. Use @lonli-lokli/dynamic-forms-validator-ajv6 if you need to validate against invalid schemas.
- Fix additionalProperties validation [#3213](https://github.com/rjsf-team/react-jsonschema-form/issues/3213)
- Report all schema errors thrown by Ajv. Previously, we would only report errors thrown for a missing meta-schema. This behavior is unchanged for @lonli-lokli/dynamic-forms-validator-ajv6.
...
- Updated the tests to use the `@lonli-lokli/dynamic-forms-validator-ajv8` fixing [#3110](https://github.com/rjsf-team/react-jsonschema-form/issues/3110)

## @lonli-lokli/dynamic-forms-bootstrap
...
- Deprecated this library in favor of the `@lonli-lokli/dynamic-forms-validator-ajv8`
...
- Updated the `validation` guide to document the new `AjvClass` prop on `CustomValidatorOptionsType` and mentioning the deprecation of `@lonli-lokli/dynamic-forms-validator-ajv6`
...
- Updated all the documentation to switch to Typescript notation where missing along with switching to using the `@lonli-lokli/dynamic-forms-validator-ajv8` validator as the default
...
- Add stubbed `WrapIfAdditionalTemplate`. `additionalProperties` is currently not supported in `@lonli-lokli/dynamic-forms-fluent-ui` (See [#2777](https://github.com/rjsf-team/react-jsonschema-form/issues/2777)).
...
- Updated `Theme` to use the renamed `ThemeProps` from `@lonli-lokli/dynamic-forms-core`
- Pass `uiSchema` appropriately to all of the `IconButton`s, `ArrayFieldItemTemplate` and `WrapIfAdditional` components, fixing (https://github.com/rjsf-team/react-jsonschema-form/issues/3130)
...
- Added tests for the new `@lonli-lokli/dynamic-forms-validator-ajv8` to the `validate_test.js` file to ensure the validation works with both validator implementations
...
- Added peer dependencies to new `@lonli-lokli/dynamic-forms-utils` library now that it is published on npm
...
- Refactored all themes to use the new `@lonli-lokli/dynamic-forms-utils` library functions and types
...
- Refactoring `utils.js` into the new `@lonli-lokli/dynamic-forms-utils` fixes (https://github.com/rjsf-team/react-jsonschema-form/issues/2719)
...
- The theme for Material UI version 5 (i.e. `@lonli-lokli/dynamic-forms-mui`) was split out of the theme for version 4 (i.e. `@lonli-lokli/dynamic-forms-material-ui`) to resolve the following issues:
...
- fix(@lonli-lokli/dynamic-forms-chakra-ui): append SubmitButton by @terrierscript in https://github.com/rjsf-team/react-jsonschema-form/pull/2860
...
- fix(@lonli-lokli/dynamic-forms-bootstrap-4): Change custom attribute to bsPrefix by @WillowP in https://github.com/rjsf-team/react-jsonschema-form/pull/3049
...
- Fixed bundler warning issue (#2762) by exporting a `@lonli-lokli/dynamic-forms-material-ui/v4` and `@lonli-lokli/dynamic-forms-material-ui/v5` sub-package
  - NOTE: `@lonli-lokli/dynamic-forms-material-ui` was retained to avoid a breaking change, but using it will continue to cause bundler warnings
  - See the `README.md` for the `@lonli-lokli/dynamic-forms-material-ui` package for updated usage information
...
- Fixed the `toIdSchema()` typescript definition to add new `idSeparator` prop along with the spelling of `idPrefix`
  ...
  - Updated `ArrayField` in `@lonli-lokli/dynamic-forms-core` to pass `idSeparator` and `idPrefix` through to `SchemaField`, fixing a small bug
...
- Bumped the peer dependencies of `@lonli-lokli/dynamic-forms-core` to `^4.0.0` for all of themes in `package.json`
...
- Add default Form export to @lonli-lokli/dynamic-forms-antd (https://github.com/rjsf-team/react-jsonschema-form/pull/2514)
